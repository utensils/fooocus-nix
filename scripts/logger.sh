#!/usr/bin/env bash
# logger.sh: Logging utilities for Fooocus launcher
...
display_url_info() {
    echo -e "\n${BLUE}-------------------------------------------${RESET}" >&2
    echo -e "${GREEN}Fooocus URL: ${RESET}http://127.0.0.1:$FOOOCUS_PORT" >&2
    echo -e "${BLUE}-------------------------------------------${RESET}" >&2
}
...
display_notices() {
    ...
    echo -e "\nTo open manually: open http://127.0.0.1:$FOOOCUS_PORT" >&2
}
